...
def flash_attention_kernel(task_args: Dict[str, int], io_tensors: Dict[str, kdt.Tile]):
    ...
    QK_score = kdt.alloc_spm((BLOCK_SIZE_QO, BLOCK_SIZE_KV), dtype='float32')
    ...
    global_rowsum = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32', init_value=0)

    local_rowmax = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32')
    local_rowsum = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32')

    scale_online_softmax = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32')
    
    global_rowmax_new = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32')
    global_rowsum_new = kdt.alloc_spm((BLOCK_SIZE_QO,), dtype='float32')
    ...
    for kv_id in range(0, NUM_STAGES):
        ...
        if kv_id == 0:
            kdt.matmul(Q_tile, kdt.transpose(K_tiles[0], 0, 1), QK_score, accumulate=False)
    ...
    for kv_id in range(0, num_kv_blocks):
        ...
        kdt.reduce(QK_score, 1, 'max', local_rowmax)
        kdt.max(local_rowmax, global_rowmax, global_rowmax_new)
        
        kdt.sub(QK_score, kdt.broadcast_to(kdt.unsqueeze(global_rowmax_new, 1), 1, BLOCK_SIZE_KV), QK_score)
        kdt.exp(QK_score, QK_score, e)
        kdt.matmul(QK_score, V_tile, V_tile)
        PV_tile = V_tile

        # PV_tile = matmul(..., ...) overlap w/ VXM ops under
        kdt.reduce(QK_score, 1, 'sum', local_rowsum)
        ...
        if next_kv_id < num_kv_blocks:
            ...
            # type: MXM
            kdt.matmul(Q_tile, kdt.transpose(K_tiles[next_stage], 0, 1), QK_score, accumulate=False)
