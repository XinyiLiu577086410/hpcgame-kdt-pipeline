...
class ASTVisitor(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign) -> Any:
        ...
        if current_scope.parent is not None and current_scope.parent.lookup(var_name) is not None and os.getenv("KDT_ENABLE_VARIABLE_SHADOWING", "0") != "1":
            raise UnsupportedFeatureError("Currently KDT-DSL does not support variable shadowing (variables in different scopes having the same name)")
        if current_scope.lookup(var_name, omit_parent=True):
            raise UnsupportedFeatureError("Currently KDT-DSL does not support variable re-assignment. You may use IfExp (like `a = val0 if k == 0 else val1`) for scalars or indexing for tiles")
    ...
    def _handle_kdt_instruction(self, node: ast.Call) -> ir.Instruction:
        ...
        if method_name in self._KDT_UNARY_INSTRUCTIONS:
            ...
            args = self._parse_func_call_args(node, [
                FuncArg("x", InterpretOptions.NONE),
                FuncArg("msg", InterpretOptions.STRING, ""),
                FuncArg("print_only_if_job0", InterpretOptions.CONST_BOOL, False)
            ])
            tile_or_expr = self._visit_expr(args['x'])
            if not isinstance(tile_or_expr, ir.Tile) and not isinstance(tile_or_expr, ir.Expr):
                raise TypeError("`kdt.print`'s first argument must be a Tile or an Expr", node)
            instr = ir.Print(
                tile_or_expr,
                args['msg'],
    ...
    def _visit_ifexp(self, node: ast.IfExp) -> ir.SelectionExpr:
        """Handle inline if expression (ternary operator)."""
        cond = self._resolve_expr(node.test)
        true_expr = self._visit_expr(node.body)
        false_expr = self._visit_expr(node.orelse)

        if isinstance(true_expr, ir.Tile) and isinstance(false_expr, ir.Tile):
            raise UnsupportedFeatureError(
                """
                Currently KDT-DSL does not support inline if expressions that return tiles.
                ...
                a0 = alloc_spm((10, ))
                a1 = alloc_spm((10, ))
                a = a0 if cond else a1  # This is not supported!
                ...
                (note: Inline if expressions among expressions (instead of tiles) are supported.)
                """,
                node
            )
        elif isinstance(true_expr, ir.Expr) and isinstance(false_expr, ir.Expr):
            return ir.SelectionExpr(cond, true_expr, false_expr, loc=IRLoc(node))
        else:
            raise TypeError(
                f"Unsupported types in inline if expression: {type(true_expr)} vs {type(false_expr)}",
                node
            )
