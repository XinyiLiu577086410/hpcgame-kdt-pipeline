...
class SelectionExpr(Expr):
    """Selection expression: cond ? true_expr : false_expr"""
    def __init__(self, cond: Expr, true_expr: Expr, false_expr: Expr, **kwargs):
        super().__init__(**kwargs)
        self.cond = cond
    ...
    def __repr__(self):
        return f"SelectionExpr({self.cond}, {self.true_expr}, {self.false_expr})"
    
    def __str__(self) -> str:
        return f"({self.cond} ? {self.true_expr} : {self.false_expr})"
    
    def as_constant(self) -> Optional[ExprValueT]:
        cond_val = self.cond.as_constant()
        if cond_val is None:
            return None
        if cond_val:
            return self.true_expr.as_constant()
        else:
            return self.false_expr.as_constant()
...
class Print(Instruction):
    ...
    """
    def __init__(self, tile_or_expr: Union[Tile, Expr], msg: str, print_only_if_job0: bool, **kwargs):
        super().__init__(**kwargs)
        self.tile_or_expr = tile_or_expr
        self.msg = msg
        self.print_only_if_job0 = print_only_if_job0

    def __repr__(self):
        return f"Print(tile_or_expr={self.tile_or_expr}, msg={self.msg}, print_only_if_job0={self.print_only_if_job0})"
